...
//Course info componenet
    ...
    public SystemClass() {
        ...
        ImageIcon universityImage = new ImageIcon("C:\\Users\\Admin\\Documents\\NetBeansProjects\\University Management System\\usal1.jpg"); // Replace with the actual path to the image
        ...
        CourseInfoB.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e
                ...
                if (cIndex != 0) {
                    ...
                    if (Allcourses.get(cIndex) instanceof CourseSection c) {
                        if (Allcourses.get(cIndex) instanceof onlineCourse course) {
                            String status = course.Status;
                            String sectionNB = String.valueOf(course.section_Nb);
                            ICStatusT.setText(status);
                            SectionNBT.setText(sectionNB);

                        }
                        if (Allcourses.get(cIndex) instanceof normalCourse course) {
                            String status = course.Status;
                            String sectionNB = String.valueOf(course.section_Nb);
                            ICStatusT.setText(status);
                            SectionNBT.setText(sectionNB);
                        ...
                        if (c.grades.isEmpty()) {
                            ...
                            AverageGradeT.setText(String.valueOf(c.getAvarageGrade()));
                        }
                    }
                } else {
        ...
        okFb.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (cIndex != 0) {
                    ...
                    if (Allcourses.get(cIndex) instanceof CourseSection c) {
                        if (c.regStud_course.isEmpty()) {
                            JOptionPane.showMessageDialog(null, "There are no students in this course", "Error Message", JOptionPane.WARNING_MESSAGE);
                        } else {
                            if (c.grades.size() == 0) {
                                for (int i = 0; i < c.regStud_course.size(); i++) {
                                    Double grade = NaN;
                                    do {
                                        grade = Double.parseDouble(JOptionPane.showInputDialog(null, c.regStud_course.get(i).Sname + "'s grade"));
                                    } while (grade.isNaN());
                                    /*{
                    ...
                    if (Allcourses.get(cIndex) instanceof CourseSection c) {
                        if (!c.grades.isEmpty()) {
                            ...
                            for (int i = 0; i < c.grades.size(); i++) {
                                JOptionPane.showMessageDialog(null, c.regStud_course.get(i).Sname + ": " + c.grades.get(i), "Grades", JOptionPane.WARNING_MESSAGE);
                            }
                            c.print_Student();
                        } else {
